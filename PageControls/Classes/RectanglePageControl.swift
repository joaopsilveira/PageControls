...
@IBDesignable open class RectanglePageControl: UIView {
    ...
    @IBInspectable open var rectangleSize: CGSize = CGSize(width: 20, height: 2.5) {
        didSet {
    ...
    fileprivate func layoutActivePageIndicator(_ progress: CGFloat) {
        ...
        let denormalizedProgress = progress * (rectangleSize.width + indicatorPadding)
    ...
    override open func sizeThatFits(_ size: CGSize) -> CGSize {
        return CGSize(width: CGFloat(inactiveLayers.count) * rectangleSize.width + CGFloat(inactiveLayers.count - 1) * indicatorPadding,
